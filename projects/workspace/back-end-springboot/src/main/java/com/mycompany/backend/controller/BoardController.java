...
public class BoardController {
	...
	@GetMapping("/list")
	public Map<String, Object> lsit (@RequestParam(defaultValue="1") int pageNo){
		log.info("실행");
	...
	public Board create(Board board) {
		...
		if(board.getBattach() != null && !board.getBattach().isEmpty()) {
			...
			try {
				File file = new File("/Users/jbc/Desktop/osstem/upload/" + board.getBattachoname());
				mf.transferTo(file);
			} catch(Exception e) {
	...
	//전체 수정 //전체 수정은 put, 일부 수정은 fetch이다. 
	@PutMapping("/")
	...
	//http://localhost/board/3 의 형식을 가진다. 
	@GetMapping("/{bno}")
	...
	//삭제 //중요하지 않은 정보만 이렇게 하고 그 외는 post로 한다. 
	@DeleteMapping("/{bno}")
	...
	public ResponseEntity<InputStreamResource> download(@PathVariable int bno) throws Exception{
		...
		if(battachoname == null) {
			...
			//파일 입력 스트림 생성
			FileInputStream fis = new FileInputStream("/Users/jbc/Desktop/osstem/upload/" + board.getBattachsname());
