...
import java.sql.*; //JDBC 라이브러리(API) - 인터페이스를 사용한다. 
import java.util.*;
...
public class ConnectExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String sql = "select employee_id, email, salary, hire_date FROM employees WHERE employee_id = 100"; //sql명령
			...
			while(rs.next()) {
				...
				System.out.println(empId + "," + email + "," + salary + "," + hire_date);
			}
